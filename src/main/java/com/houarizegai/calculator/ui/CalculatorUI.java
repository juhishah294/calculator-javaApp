...
public class CalculatorUI {
    ...
    private void initButtons(int[] columns, int[] rows) {
        ...
        btnEqual.addActionListener(event -> {
            ...
            if (go) {
                double result = calculate(typedValue, Double.parseDouble(inputScreen.getText()), selectedOperator);
        ...
        btnPower = createButton("^", columns[4], rows[1]);
        ...
        btnRoot = createButton("√", columns[4], rows[2]);
        btnRoot.addActionListener(event -> {
            if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()))
                return;

            double number = Double.parseDouble(inputScreen.getText());
            inputScreen.setText(String.valueOf(Math.sqrt(number)));
        });
        ...
        btnLog.addActionListener(event -> {
            ...
            double number = Double.parseDouble(inputScreen.getText());
            inputScreen.setText(String.valueOf(Math.log(number)));
        });
    }
    ...
    private JComboBox<String> createComboBox(String[] items, int x, int y, String title) {
        JLabel label = new JLabel(title);
        label.setBounds(x, y, 80, 30);
        window.add(label);

        JComboBox<String> comboBox = new JComboBox<>(items);
        comboBox.setBounds(x, y + 30, 150, 30);
    ...
    private void applyTheme(Theme theme) {
        window.getContentPane().setBackground(hex2Color(theme.getBackground()));
        inputScreen.setBackground(hex2Color(theme.getScreenColor()));
        inputScreen.setForeground(hex2Color(theme.getFontColor()));
        inputScreen.setCaretColor(hex2Color(theme.getCaretColor()));

        for (JButton button : getAllButtons()) {
            button.setBackground(hex2Color(theme.getButtonColor()));
            button.setForeground(hex2Color(theme.getFontColor()));
        }

        comboCalculatorType.setBackground(hex2Color(theme.getComboBoxColor()));
        comboCalculatorType.setForeground(hex2Color(theme.getFontColor()));

        comboTheme.setBackground(hex2Color(theme.getComboBoxColor()));
        comboTheme.setForeground(hex2Color(theme.getFontColor()));
    }

    private JButton[] getAllButtons() {
        return new JButton[] { btnC, btnBack, btnMod, btnDiv, btnMul, btnSub, btnAdd, btn0, btn1, btn2, btn3, btn4,
